...
def initializeGroverSearch() -> tuple[dict, dict, AerSimulator]:
    """
    ...
    print("Quantum Search Configuration:")
...
def constructGroverCircuit(
    ...
    if n_iterations is None:
        ...
        optimalIterations = int(np.round(np.pi / (4 * np.arcsin(1 / np.sqrt(search_space_size)))))
    ...
    for iteration in range(n_iterations):
        ...
        # Multi-controlled Z: flip phase of |1...1> state
        if n_qubits == 1:
            circuit.z(qubits[0])
        elif n_qubits == 2:
            ...
            # Multi-controlled phase gate: exact MCZ without ancilla
