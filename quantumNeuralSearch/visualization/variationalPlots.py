...
def visualizeVariationalResults(
    results, cost_history, seqCmap, divCmap, cubehelix_reverse, save_path=None
) -> plt.Figure:
    """
    ...
    quantum_colors = plt.cm.viridis(np.linspace(0, 1, len(state_names)))  # type: ignore[attr-defined]
    performance_colors = plt.cm.plasma(np.linspace(0.2, 0.8, 4))  # type: ignore[attr-defined]
    ...
    _stdParam = np.std(weights)
    ax3.axvline(
        mean_param, color="red", linestyle="--", linewidth=2, label=f"Mean: {mean_param:.3f}"
    )
    ax3.legend()
    ...
    advantage_categories = ["VQC", "SVM", "Random\nForest", "Random\nBaseline"]
    svmAcc = results.get("svm_accuracy", 1.0 / len(state_names))
    rfAcc = results.get("rf_accuracy", 1.0 / len(state_names))
    randomBaseline = 1.0 / len(state_names)
    scores = [results["test_accuracy"], svmAcc, rfAcc, randomBaseline]
    barColors = [
        performance_colors[2],
        performance_colors[1],
        performance_colors[0],
        performance_colors[3],
    ]

    x_pos = np.arange(len(advantage_categories))

    ax7.bar(x_pos, scores, 0.5, color=barColors)

    ax7.set_title(
        "Classifier Comparison\n(Same Train/Test Split)", fontsize=14, fontweight="bold", pad=25
    )
    ...
    fig.suptitle(
        "Variational Quantum Algorithm for Brain State Classification",
        fontsize=20,
        fontweight="bold",
        y=0.96,
    )
    ...
    fig.text(
        ...
        f"Quantum Advantage: {results['quantum_improvement']:.1f}x | "
        f"Circuit Depth: {results['circuit_complexity']['circuit_depth']}",
        ha="center",
        va="center",
        fontsize=14,
        color="gray",
...
def createVariationalPerformanceSummary(results, cost_history) -> str:
    """
    ...
    summary += f"  \u2022 Best Performing State: {bestState}\n"
...
if __name__ == "__main__":
    ...
    fig = visualizeVariationalResults(
        results,
        cost_history,
        seqCmap,
        divCmap,
