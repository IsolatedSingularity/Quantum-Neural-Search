...
def visualizeGroversResults(
    classification_results, seqCmap, divCmap, cubehelix_reverse, save_path=None
) -> plt.Figure:
    """
    ...
    sorted_data = sorted(
        zip(pattern_labels, pattern_counts, pattern_colors), key=lambda x: x[1], reverse=True
    )
    if sorted_data:
        ...
        ax4.bar(range(display_n), display_counts, color=display_colors, alpha=0.8)

        ax4.set_title(
            f"Measurement Results for {best_state.replace('_', ' ').title()}\n"
            + f"Target Pattern: {best_results['target_pattern']} "
            + f"(Success Rate: {best_results['success_probability']:.1%})",
            fontsize=14,
            fontweight="bold",
        )
    ...
    state_patterns = np.array(
        [classification_results[name]["target_pattern"] for name in state_names]
    )
    ...
    wedges, texts, autotexts = ax7.pie(  # type: ignore[misc]
        quantum_advantages,
    ...
    plt.suptitle(
        "Grover's Algorithm for Quantum Brain State Classification\n"
...
def createGroverPerformanceSummary(classification_results, performance_metrics) -> str:
    """
    ...
    summary += (
        f"  \u2022 High-Fidelity States: "
        f"{performance_metrics['high_fidelity_states']}/{performance_metrics['total_states']}\n"
    )
    summary += (
        f"  • Mean Quantum Advantage: {performance_metrics['mean_quantum_advantage']:.1f}x\n\n"
    )

    summary += "Individual Brain State Performance:\n"
    ...
    summary += "  • Search Space Size: 16 states (4 qubits)\n"
    optimalIter = np.mean([r["iterations"] for r in classification_results.values()])
    summary += f"  \u2022 Optimal Iterations: {optimalIter:.1f}\n"
