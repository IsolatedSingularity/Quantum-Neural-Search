...
from quantumNeuralSearch.brainAtlas import (
    createBrainAtlas,
    ensurePlotsDirectory,
    initializeVisualizationSettings,
)
from quantumNeuralSearch.brainConnectivity import (
    enhanceNetworkConnectivity,
    generateBrainConnectivity,
    visualizeConnectivityMatrix,
)
from quantumNeuralSearch.dynamicStates import createBrainNetworkAnimation
...
from quantumNeuralSearch.neuralEncoding import (
    ...
    createQuantumNeuroscienceVisualization,
)
...
def runComprehensiveBrainAnalysis(save_all_plots=True, create_animations=True) -> dict:
    """
    ...
    connectivity_fig = visualizeConnectivityMatrix(
        edges,
        atlasinfo,
        seqCmap,
    ...
    try:
        saved_3d_files = create3dBrainVisualization(
            nodes_df_coords,
            atlasinfo,
            enhanced_edges,
    ...
    if create_animations:
        ...
        try:
            anim, anim_fig = createBrainNetworkAnimation(
                enhanced_edges,
                atlasinfo,
                seqCmap,
                divCmap,
    ...
    quantum_neuroscience_fig = createQuantumNeuroscienceVisualization(
        seqCmap,
        divCmap,
        cubehelix_reverse,
        f"{plots_dir}/master_quantum_neuroscience.png" if save_all_plots else None,
    )
    quantum_circuit_fig = createQuantumCircuitVisualization(
        seqCmap,
        divCmap,
        cubehelix_reverse,
    ...
    grover_fig = visualizeGroversResults(
        grover_results,
        seqCmap,
        divCmap,
        cubehelix_reverse,
    ...
    vqc_results = evaluateQuantumClassifier(
        X_train, X_test, y_train, y_test, vqc_weights, quantum_classifier, state_names
    )

    # Visualize Variational results
    vqc_fig = visualizeVariationalResults(
        vqc_results,
        vqc_cost_history,
        seqCmap,
        divCmap,
...
def createMasterSummaryVisualization(
    atlasinfo,
    edges,
    grover_results,
    grover_performance,
    ...
    ax2.bar(range(len(network_names)), network_sizes, color=colors)
    ...
    ax3.bar(range(len(grover_states)), grover_probs, color=divCmap(0.6))
    ...
    ax4.bar(range(len(vqc_states)), vqc_accs, color=cubehelix_reverse(0.6))
    ...
    quantum_advantage = [
        grover_performance["mean_quantum_advantage"],
        vqc_results["quantum_improvement"],
    ]
    ...
    ax5.bar(
        x_pos - width, quantum_performance, width, label="Quantum Performance", color=seqCmap(0.7)
    )
    ax5.bar(x_pos, classical_baseline, width, label="Classical Baseline", color=divCmap(0.5))
    ax5.bar(
        x_pos + width,
        np.array(quantum_advantage) / 10,
        width,
        label="Quantum Advantage (÷10)",
        color=cubehelix_reverse(0.7),
    )
    ...
    ax6.bar(
        x_pos - width / 2, classical_ops, width, label="Classical Operations", color=divCmap(0.6)
    )
    ax6.bar(x_pos + width / 2, quantum_ops, width, label="Quantum Operations", color=seqCmap(0.7))
    ...
    hfStates = grover_performance["high_fidelity_states"]
    totalStates = grover_performance["total_states"]
    groverAdv = grover_performance["mean_quantum_advantage"]
    nParams = vqc_results["circuit_complexity"]["n_parameters"]
    nQubits = vqc_results["circuit_complexity"]["n_qubits"]
    vqcImprv = vqc_results["quantum_improvement"]
    bestGroverState = max(
        grover_results,
        key=lambda x: grover_results[x]["success_probability"],
    )
    bestVqcState = max(
        vqc_results["state_accuracies"],
        key=vqc_results["state_accuracies"].get,
    )
...
• Quantum Speedup: {groverAdv:.1f}x over classical search
• High-Fidelity States: {hfStates}/{totalStates} states achieve >70% accuracy
• Best Performing State: {bestGroverState}

Variational Quantum Classifier Results:
• Test Accuracy: {vqc_results["test_accuracy"]:.1%}
• Quantum Improvement: {vqcImprv:.1f}x over random baseline
• Circuit Complexity: {nParams} parameters on {nQubits} qubits
• Best Performing State: {bestVqcState}

Overall Quantum Advantage:
• Pattern Search: {groverAdv:.1f}x speedup for exact pattern matching
• Machine Learning: {vqcImprv:.1f}x improvement for adaptive classification
...
• Near-term Feasibility: Compatible with current NISQ hardware capabilities"""
    ...
    # Main title
    fig.suptitle(
        "Comprehensive Quantum Neuroscience Analysis\n"
        "Brain Networks, Grover's Search, "
...
def printComprehensiveResults(results) -> None:
    """
    ...
    print(
        f"  \u2022 Functional Networks: "
        f"{len(results['brain_atlas']['atlasinfo']['yeo7networks'].unique())}"
    )
    ...
    print(
        ...
        f"{grover_perf['high_fidelity_states']}/{grover_perf['total_states']}"
    )
