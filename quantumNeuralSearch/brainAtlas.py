...
def initializeVisualizationSettings() -> tuple:
    """
    ...
    cubehelix_reverse = sns.cubehelix_palette(
...
def createBrainAtlas() -> tuple:
    ...
    network_labels = (
        ["DMN"] * 6 + ["SN"] * 5 + ["CEN"] * 6 + ["SMN"] * 6 + ["VIS"] * 6 + ["AUD"] * 4
    )
    ...
    atlasinfo = pd.DataFrame(
        {
            ...
            "hemisphere": [
                "L" if "_L" in name else "R" if "_R" in name else "M" for name in brain_regions
            ],
        }
    )

    # Generate realistic 3D coordinates for brain regions (MNI space approximation)
    rng = np.random.default_rng(42)  # For reproducible coordinates
    ...
    for i, (name, network) in enumerate(zip(brain_regions, network_labels)):
        ...
        # Add some variation
        coord = [
            base[0] + rng.normal(0, 5),
            base[1] + rng.normal(0, 5),
            base[2] + rng.normal(0, 5),
        ]
        coords.append(coord)
    ...
    nodes_df_coords = pd.DataFrame({"x": coords[:, 0], "y": coords[:, 1], "z": coords[:, 2]})  # type: ignore[call-overload]

    # Use the network labels we already defined
    atlasinfo["yeo7networks"] = atlasinfo["network"]

    print(
        f"Atlas created: {len(atlasinfo)} brain regions "
        f"across {len(atlasinfo['yeo7networks'].unique())} networks"
    )
